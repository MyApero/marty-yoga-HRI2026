...
def apply_film_effect(frame, settings):
    ...
    film = cv2.convertScaleAbs(gray, alpha=settings['contrast'], beta=settings['brightness'])
...
def draw_skeleton(image, pose_landmarks, config):
    ...
    for pose in pose_landmarks:
        ...
        for joint in fb_settings['joints_to_monitor']:
            p1, p2, p3 = pose[joint['points'][0]], pose[joint['points'][1]], pose[joint['points'][2]]
        ...
        cv2.line(image, pt_nose, pt_sh_center, default_color, f_settings['line_thickness'])
        cv2.line(image, pt_sh_center, pt_hip_center, default_color, f_settings['line_thickness'])
        ...
        limb_to_joint = {
            13: "L-Elbow", 15: "L-Elbow", 17: "L-Elbow", 19: "L-Elbow", 21: "L-Elbow",
            14: "R-Elbow", 16: "R-Elbow", 18: "R-Elbow", 20: "R-Elbow", 22: "R-Elbow",
            25: "L-Knee", 27: "L-Knee", 23: "L-Hip",
            26: "R-Knee", 28: "R-Knee", 24: "R-Hip"
        }
        ...
        for start_idx, end_idx in s_settings['body_lines'] + s_settings['hand_lines']:
            ...
            if start.presence > 0.5 and end.presence > 0.5:
                ...
                line_color = joint_status[j_name]['color'] if j_name in joint_status else default_color

                if start_idx in [11, 12]: p1 = pt_sh_center
                elif start_idx in [23, 24]: p1 = pt_hip_center
                else: p1 = (int(start.x * w), int(start.y * h))
        ...
        for name, data in joint_status.items():
            ...
            label = f"{name}: {data['angle']}°" if fb_settings['show_labels'] else f"{data['angle']}°"
            ...
            cv2.putText(image, label, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 
                        fb_settings['text_scale'], data['color'], 1)
...
def main():
    ...
    while cap.isOpened():
        ...
        if result.pose_landmarks:
            draw_skeleton(output_frame, result.pose_landmarks, config)

        big_frame = cv2.resize(output_frame, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
